...
import { BookOpen, CheckCircle, Clock, Target, Zap, ChevronLeft, Home, RefreshCw, AlertCircle, XCircle, Check } from 'lucide-react';
...
const GrammarTest: React.FC<GrammarTestProps> = ({ userId, onBack }) => {
  ...
  const startTest = useCallback(() => {
    const questions = getRandomQuestions(15, 'grammar'); // 减少到15题以优化体验，但覆盖不同知识点
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <Card className="text-center border-none shadow-sm">
          <CardContent className="pt-8 pb-8">
            ...
            <h2 className="text-2xl font-bold text-foreground mb-4">语法诊断测试</h2>
            <p className="text-muted-foreground mb-6">
              全面测试你的英语语法掌握程度，包含时态、从句、非谓语等核心知识点。<br/>
              测试完成后将生成详细的知识点诊断报告。
            </p>
            
            <div className="grid grid-cols-2 gap-4 mb-8 text-left bg-muted/50 rounded-xl p-4">
              <div className="flex items-center gap-2"><Clock className="w-4 h-4 text-muted-foreground" /><span className="text-sm text-foreground">约 5-8 分钟</span></div>
              <div className="flex items-center gap-2"><Target className="w-4 h-4 text-muted-foreground" /><span className="text-sm text-foreground">15 道精选题</span></div>
              <div className="flex items-center gap-2"><Zap className="w-4 h-4 text-muted-foreground" /><span className="text-sm text-foreground">多维度考点</span></div>
              <div className="flex items-center gap-2"><BookOpen className="w-4 h-4 text-muted-foreground" /><span className="text-sm text-foreground">详细诊断报告</span></div>
            </div>
      ...
      <div className="max-w-3xl mx-auto">
        <Card className="border-none shadow-sm mb-6">
          <CardContent className="pt-8 pb-8 text-center">
            ...
            <p className="text-muted-foreground mb-8">你的语法能力综合评估</p>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-red-700 dark:text-red-300">
                      你在 <strong>{weakPoints.join('、')}</strong> 方面存在薄弱环节。建议前往「专项练习」进行针对性训练，巩固相关语法规则。
                    </p>
                  </div>
                </div>
              </div>
