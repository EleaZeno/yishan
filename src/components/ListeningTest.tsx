...
import { Headphones, CheckCircle, Clock, ChevronLeft, Home, RefreshCw, XCircle, Check, Target, AlertCircle, Volume2 } from 'lucide-react';
import { getRandomQuestions, type Question } from '../data/questions';
import { Card, CardContent } from './ui/card';
import { Button } from './ui/button';
import { Progress } from './ui/progress';
import { updateKnowledgeMastery } from '../services/assessment';
...
const ListeningTest: React.FC<ListeningTestProps> = ({ userId, onBack }) => {
  ...
  const startTest = useCallback(() => {
    const questions = getRandomQuestions(5, 'listening');
  ...
  const handleAnswer = async (selectedIndex: number) => {
    ...
    if (userId) {
      try {
        await updateKnowledgeMastery(currentQ.knowledgeId, isCorrect, 0.5, userId);
      } catch (e) {
    ...
    setTimeout(() => {
      if (currentIndex >= shuffledQuestions.length - 1) {
        ...
        newAnswers.forEach(ans => {
          const pointName = KNOWLEDGE_GRAPH[ans.question.knowledgeId]?.name || '其他听力';
  ...
  const playAudio = (text: string) => {
    // Extract the audio part from the content
    const audioMatch = text.match(/\(Audio: (.*?)\)/);
    const textToSpeak = audioMatch ? audioMatch[1] : text;
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <Card className="text-center border-none shadow-sm">
          <CardContent className="pt-8 pb-8">
            ...
            <h2 className="text-2xl font-bold text-foreground mb-4">听力理解诊断测试</h2>
            <p className="text-muted-foreground mb-6">
              全面测试你的英语听力理解能力，包括细节获取、主旨大意、推理判断等。<br/>
              测试完成后将生成详细的听力能力诊断报告。
            </p>
            
            <div className="grid grid-cols-2 gap-4 mb-8 text-left bg-muted/50 rounded-xl p-4">
              <div className="flex items-center gap-2"><Clock className="w-4 h-4 text-muted-foreground" /><span className="text-sm text-foreground">约 5 分钟</span></div>
              <div className="flex items-center gap-2"><Headphones className="w-4 h-4 text-muted-foreground" /><span className="text-sm text-foreground">5 道听力题</span></div>
              <div className="flex items-center gap-2"><Check className="w-4 h-4 text-muted-foreground" /><span className="text-sm text-foreground">主旨/细节/推理</span></div>
      ...
      <div className="max-w-3xl mx-auto">
        <Card className="border-none shadow-sm mb-6">
          <CardContent className="pt-8 pb-8 text-center">
            ...
            <p className="text-muted-foreground mb-8">你的听力能力综合评估</p>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-red-700 dark:text-red-300">
                      你在 <strong>{weakPoints.join('、')}</strong> 题型上存在薄弱环节。建议在后续练习中重点关注此类题型的听力技巧。
                    </p>
                  </div>
                </div>
              </div>
  ...
  const questionText = currentQ.content.replace(/\(Audio: .*?\)/, '').trim();
      ...
      <Card className="border-none shadow-sm">
        <CardContent className="pt-8 pb-8">
          <div className="inline-block bg-primary/10 text-primary px-3 py-1 rounded-full text-sm font-medium mb-4">
            {KNOWLEDGE_GRAPH[currentQ.knowledgeId]?.name || '听力'}
          </div>
          
          <div className="bg-muted/50 rounded-xl p-6 mb-6 flex flex-col items-center gap-4">
            <Button 
              size="lg" 
              className="rounded-full w-16 h-16 flex items-center justify-center"
              onClick={() => playAudio(currentQ.content)}
            >
              <Volume2 className="w-8 h-8" />
            </Button>
            <p className="text-muted-foreground text-sm">点击播放录音</p>
            <p className="text-foreground leading-relaxed text-lg font-medium mt-4">{questionText}</p>
          </div>
