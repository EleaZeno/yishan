...
import { TestTube, CheckCircle, Clock, ChevronLeft, Home, RefreshCw, XCircle, Check, Target, AlertCircle } from 'lucide-react';
...
const ReadingTest: React.FC<ReadingTestProps> = ({ userId, onBack }) => {
  ...
  const startTest = useCallback(() => {
    const questions = getRandomQuestions(5, 'reading'); // 减少到5题以优化体验
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <Card className="text-center border-none shadow-sm">
          <CardContent className="pt-8 pb-8">
            ...
            <h2 className="text-2xl font-bold text-foreground mb-4">阅读理解诊断测试</h2>
            <p className="text-muted-foreground mb-6">
              全面测试你的英语阅读理解能力，包括细节理解、主旨大意、推理判断等。<br/>
              测试完成后将生成详细的阅读能力诊断报告。
            </p>
            
            <div className="grid grid-cols-2 gap-4 mb-8 text-left bg-muted/50 rounded-xl p-4">
              <div className="flex items-center gap-2"><Clock className="w-4 h-4 text-muted-foreground" /><span className="text-sm text-foreground">约 10 分钟</span></div>
              <div className="flex items-center gap-2"><TestTube className="w-4 h-4 text-muted-foreground" /><span className="text-sm text-foreground">5 篇短文</span></div>
      ...
      <div className="max-w-3xl mx-auto">
        <Card className="border-none shadow-sm mb-6">
          <CardContent className="pt-8 pb-8 text-center">
            ...
            <p className="text-muted-foreground mb-8">你的阅读能力综合评估</p>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-red-700 dark:text-red-300">
                      你在 <strong>{weakPoints.join('、')}</strong> 题型上存在薄弱环节。建议在后续练习中重点关注此类题型的解题技巧。
                    </p>
                  </div>
                </div>
              </div>
